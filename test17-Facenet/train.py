...
if __name__ == "__main__":
    ...
    loss = triplet_loss()
    loss_history = LossHistory("logs")# 记录loss的变化，保存在logs文件夹下
    # ---------------------------------#
    #   LFW估计
    # ---------------------------------#
    ...
    # LFWDataset返回的是两张图片和一个标签(issame，True or False)
    ...
    if True:
        ...
        # FacenetDataset对于一份batch会返回3batch_size的图片，分别是anchor，positive，negative，用于triplet_loss计算
        # 同时会返回label，进行分类loss计算
